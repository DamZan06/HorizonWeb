(function () {
    ...
    function calculateSummary({ points = [], routeMeta, now = Date.now() }) {
        ...
        const etaSpeed=recentMovingSpeedKmh||movingAverageSpeedKmh||averageSpeedKmh;
        const eta=state==='live'&&list.length>1&&etaSpeed>0.5&&remainingDistanceKm>0?now+remainingDistanceKm/etaSpeed*3600000:null;
        const heartRates=list.map(p=>Number(p.heartRate)).filter(v=>valid(v)&&v>30&&v<240);
        return {started:Boolean(list.length),state,plannedDistanceKm,plannedElevationGainM:Number(routeMeta?.elevationGainM)||null,coveredDistanceKm,remainingDistanceKm,completionPercent,actualStartTimestamp:firstPoint?.timestamp||null,elapsedTimeMs,movingTimeMs,currentSpeedKmh:valid(latestPoint?.speed)?Number(latestPoint.speed):null,averageSpeedKmh,movingAverageSpeedKmh,recentMovingSpeedKmh,maxSpeedKmh,currentAltitudeM:valid(latestPoint?.altitude)?Number(latestPoint.altitude):null,actualElevationGainM:window.HorizonStats.elevationGain(list),currentHeartRateBpm:valid(latestPoint?.heartRate)?Number(latestPoint.heartRate):null,averageHeartRateBpm:heartRates.length?heartRates.reduce((a,b)=>a+b,0)/heartRates.length:null,latestPoint,latestPointTimestamp:latestPoint?.timestamp||null,eta,points:list};
    }
    ...
    window.HorizonExpedition={calculateSummary,loadSummary};
})();
