...
const tx=source=>window.HorizonI18n?.t?.(`copy:${source}`)||source;
const tr=()=>({na:tx('Not available'),live:tx('Live'),delayed:tx('Delayed'),resting:tx('Resting'),ended:tx('End of day'),finished:tx('Finished'),prestart:tx('Not started'),recent:tx('Based on the last hour of movement'),moving:tx('Based on average moving speed')}),set=(id,v)=>{const n=document.getElementById(id);if(n)n.textContent=v},num=(v,d=1)=>new Intl.NumberFormat(L(),{minimumFractionDigits:d,maximumFractionDigits:d}).format(v),av=(v,u='',d=1)=>Number.isFinite(v)?num(v,d)+u:tr().na;
...
const age=v=>{if(!Number.isFinite(v))return tr().na;let m=Math.floor(v/60000),x=m<60?m+' min':m<1440?Math.floor(m/60)+' h':Math.floor(m/1440)+' d',marker=tx('ago');return ['de','fr'].includes(L())?marker+' '+x:x+' '+marker};

function update(s={}){
  S.summary=s;
  const p=s.latestPoint||{},r=s.routeMeta||{};
  const etaLabel=document.getElementById('metricEtaLabel');
  if(etaLabel)etaLabel.textContent=s.completedAt?tx('Arrival'):tx('Estimated arrival');
  const rows=[
    ...
    ['techFirebaseState',p.trackerState?tx(({stationary:'Stationary',moving:'Moving',paused:'Paused',stopped:'Stopped'})[String(p.trackerState).toLowerCase()]||String(p.trackerState)):s.state?tr()[s.state]||tx(String(s.state)):tr().na],
...
function charts(points){
  ...
  let q=points.filter((_,i)=>i%Math.max(1,Math.floor(points.length/400))===0),mode=document.getElementById('chartXAxisMode')?.value||'distance',labs=q.map(p=>mode==='distance'?num(Number(p.cumulativeDistanceKm||0)):new Date(p.timestamp).toLocaleString(L(),{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'})),g=0,gains=q.map((p,i)=>{if(i){let d=+p.altitude-+q[i-1].altitude;if(d>2&&d<200)g+=d}return g}),n=[tx('Speed'),tx('Altitude'),tx('Heart rate'),tx('Cumulative elevation')],defs=[['chartSpeed',n[0],q.map(p=>p.speed),'#e8953f'],['chartAltitude',n[1],q.map(p=>p.altitude),'#d9b36c'],['chartHeartRate',n[2],q.map(p=>p.heartRate),'#c36a4a'],['chartElevation',n[3],gains,'#8ca89f']];
...
function initDashboardPage(){
  document.querySelectorAll('.chart-card-head').forEach(h=>{let p=document.createElement('p');p.className='empty-state';p.textContent=tx('Telemetry will appear once tracking begins.');h.after(p)});
