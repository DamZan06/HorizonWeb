...
const supportedLanguages = runtimeConfig.supportedLanguages || ['en', 'it', 'de', 'fr'];
const defaultLanguage = runtimeConfig.defaultLanguage || 'en';
...
function getExpeditionState() {
    ...
    if (Number.isFinite(startMs) && now < startMs) return 'not-started';

    const forcedStatus = normalizeHomeStatus(window.localStorage?.getItem?.('horizon-live-status-force') || '');
    if (forcedStatus === 'completed') return 'completed';
    if (forcedStatus === 'paused' || forcedStatus === 'ended') return 'resting';
    if (forcedStatus === 'moving') return 'live';

    const lastRefresh = Number(window.__horizonLastLiveDataTs || 0);
    if (lastRefresh && now - lastRefresh > staleThresholdMs) return 'delayed';
    if (lastRefresh && now >= startMs) return 'live';
    if (now >= startMs && !lastRefresh) return 'not-started';
    return 'not-started';
}

if (window.HorizonApp?.startHomeRefreshScheduler) {
    window.HorizonApp.startHomeRefreshScheduler();
}
const i18nCatalog = {
    it: {
        common: {
        ...
        pages: {
            home: {
                ...
                countdownTitle: '31 Agosto · 04:00',
    ...
    en: {
        ...
        pages: {
            home: {
                ...
                countdownTitle: '31 August · 04:00',
...
function getFirebaseClientConfig() {
    return window.HorizonFirebaseConfig || null;
}
